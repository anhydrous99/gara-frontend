...
# Note: Sensitive values should be set via container orchestration, not here
...
# Health check
# Container orchestration platforms use this to determine container health
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD node -e "require('http').get('http://localhost:80', (r) => {if (r.statusCode !== 200) throw new Error(r.statusCode)})"

# Start Next.js server
# Logs go to stdout/stderr for container logging
